...
class MarshMellowPermissions(var mActivity: BaseFragmentActivity) {
    ...
    fun requestPermissionForReadExternalStorage() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(mActivity, Manifest.permission.READ_EXTERNAL_STORAGE)) {
            Toast.makeText(mActivity, " External Storage permission is needed. Please turn it on inside the settings", Toast.LENGTH_SHORT).show()
        } else {
            ActivityCompat.requestPermissions(mActivity, arrayOf<String>(Manifest.permission.READ_EXTERNAL_STORAGE), EXTERNAL_STORAGE_READ_PERMISSION_REQUEST_CODE)
        }
    }
